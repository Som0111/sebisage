...
- **Phase 9 done, live deployment skipped by decision.** `Dockerfile`, `start.sh`, `.dockerignore`, `.github/workflows/ci.yml` (lint/test/retrieval-gate) all written. `scripts/retrieval_gate.py` ran for real locally: PASS (MRR@10 = 0.7619, exact match to the real Phase 3 baseline). **Docker build and run verified for real** (not just written): full build succeeded (~35 min, mostly slow network), container's `/health` returned 200 with the correct baked-in index stats, Streamlit reachable too, and a real `/ask` query confirmed working end to end with peak memory ~793.5MiB (ruling out the old "16GB" assumption). One real bug found and fixed: an unneeded `apt-get install build-essential` step failed on a network issue and turned out to be unnecessary entirely (all deps ship prebuilt wheels) — removed rather than worked around. Deployment plan changed twice: HF Spaces' Docker SDK turned out to be paid-only on the human's account; the fallback, Oracle Cloud Always Free, needs a card on file for identity verification, which wasn't available — so live deployment is deferred rather than forced, and `README.md` says so honestly. Later asked to shrink the image toward a free-tier size limit (models downloaded at container startup instead of baked in, `matplotlib` moved out of runtime deps) — real result **3.6GB → 3.28GB**, re-verified working end to end. Did not reach the requested <1GB; measured why (torch/scipy/chromadb/streamlit's transitive deps are the load-bearing bulk, not the models) and reported honestly rather than force the number — human decided to stop there rather than rearchitect. Details below.
...
**Image-size follow-up (attempted, real result reported honestly):** asked to shrink the image toward a free tier's typical size limit by deferring model downloads to container startup instead of baking them into the build. Implemented and verified: rewrote `Dockerfile` as a multi-stage build (indexes are still built in a discarded `indexer` stage since they're cheap — ~27MB total — and expensive to compute — ~17 min; only the resulting `storage/` directory crosses into the final image), added `scripts/warm_models.py` to download the bge-small embedder and cross-encoder at container startup (`start.sh` runs it before `uvicorn`/`streamlit` start), and moved `matplotlib` (only used by the offline `scripts/run_ablation.py`, never imported at runtime) from the base dependency set into `dev` extras.
Rebuilt and verified for real: container starts, downloads both models fresh (confirmed in logs, not baked in), `/health` → 200 with correct index stats, a real `/ask` query → 200 with a correctly grounded, cited answer, peak memory unchanged at ~802MiB. 98/98 tests pass, ruff clean.
**Real result: 3.6GB → 3.28GB** (~9% smaller) — not the requested <1GB. Checked why before doing anything more invasive: measured actual installed-package sizes and found `torch` alone is 506MB unpacked, with `scipy` (113MB), `transformers` (102MB), `pyarrow` (86MB), `kubernetes` client (77MB, a transitive dep of `chromadb`), `sympy` (72MB), `pandas` (63MB), `onnxruntime` (44MB) and `scikit-learn` (42MB) adding up well past 1GB before any model weights or app code. These are load-bearing for the current architecture (torch for the cross-encoder reranker, Chroma's own heavy transitive deps, Streamlit pulling in pandas/pyarrow) — hitting <1GB would require swapping major components (e.g. ONNX-only inference, a lighter vector store, dropping Streamlit), not a Docker optimization. Presented this honestly instead of forcing a false number; **human decision: stop here, keep it Docker-ready but undeployed** — matches the H5 outcome above.
